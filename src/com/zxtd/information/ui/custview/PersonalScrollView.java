...
import com.zxtd.information.util.DisplayUtil;
...
public class PersonalScrollView extends ScrollView {
	...
	private float initTouchY=0;// 首次点击的Y坐标
	...
	*/
	...
	public PersonalScrollView(Context context, AttributeSet attrs) {
		...
		bageHeight= DisplayUtil.dip2px(180,scale);
	}
	...
	public void commOnTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			//}else{
			//	float scale = context.getResources().getDisplayMetrics().density; 
			//	current_Top = initTop = DisplayUtil.dip2px(-220,scale);
			//}
			//if(imageView.getBottom()!=0){
				current_Bottom = initBottom = imageView.getBottom();
			//}else{
			//	float scale = context.getResources().getDisplayMetrics().density; 
			//	current_Bottom = initBottom = DisplayUtil.dip2px(180,scale);
			//}
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (isMoveing) {
				...
				/** image_bg **/
				float image_move_H = deltaY /5 ;//10
	...
	public void animation() {
		...
		Log.e(Constant.TAG, "image left:"+imageView.getLeft()+"    top:"+initTop+"     right:"+imageView.getRight()+"     bottom:"+initBottom);
		...
		Log.e(Constant.TAG, "inner left:"+normal.left+"  top:"+normal.top+" right:"+normal.right+" bottom:"+normal.bottom);
